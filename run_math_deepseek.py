"""MATH Geometry L4-L5 experiments with DeepSeek-chat (API).

Exp 1: No skill baseline
Exp 2: With skill lifecycle

Usage:
    python3 run_math_deepseek.py --exp 1        # no-skill
    python3 run_math_deepseek.py --exp 2        # with-skill
    python3 run_math_deepseek.py --exp all      # both
    python3 run_math_deepseek.py --exp 1 --debug  # with debug
"""
...
API_KEY = os.environ.get("DS_API_KEY", "sk-DISQMJtpvWPwvub7Z4xC2IFHyzNt4gEwRB1dJ5fBzkt92wFY")
...
NUM_TRAIN = None
NUM_TEST = None
OUTPUT_BASE = os.environ.get("OUTPUT_BASE", "/data/hwt/skillmanage/exp_math_deepseek")
...
def run_no_skill(llm, bench, train_tasks, test_tasks):
    ...
    for label, tasks in [("Train", train_tasks), ("Test", test_tasks)]:
        ...
        if label == "Train":
            train_results, train_correct = results, correct
        else:
            test_results, test_correct = results, correct
...
def main():
    parser = argparse.ArgumentParser(description="MATH DeepSeek experiments")
    ...
    # DeepSeek as LLM judge
    llm = create_llm()
    llm.generate("test")
    bench._llm_client = llm
